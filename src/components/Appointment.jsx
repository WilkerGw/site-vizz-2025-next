...
export default function Exame() {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'whatsapp') {
      ...
      let formattedValue = '';
      if (numericValue.length > 0) formattedValue = `(${numericValue.slice(0, 2)})`;
      if (numericValue.length > 2) formattedValue += ` ${numericValue.slice(2, 7)}`;
      if (numericValue.length > 7) formattedValue += `-${numericValue.slice(7, 11)}`;
      setFormData({
        ...formData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://agendamento-backend-xoup.onrender.com/api/agendamento', {
        ...formData,
  ...
  for (let hour = 10; hour <= 17; hour++) {
    for (let minute = 0; minute < 60; minute += 15) {
      if ((hour === 10 && minute < 30) || (hour === 17 && minute > 15)) continue;
